...
async function postmanToOpenApi (input, output, {
  ...
  auth: optsAuth, servers, externalDocs = {}, folders = {}, itemServers = false,
  ...
  for (let [i, element] of items.entries()) {
    ...
    if (element != null) {
      ...
      if (valid) {
        ...
        const domain = calculateDomains(protocol, host, port)
        domains.add(domain)
        const rawUrl = (typeof url === 'string' || url instanceof String) ? url : url.raw
        const servers =
          itemServers && rawUrl && rawUrl.startsWith(protocol) ? [{ url: domain }] : undefined
        ...
        console.error('Invalid item ignored:', name, method, url || tag)
      }
    }
  }
