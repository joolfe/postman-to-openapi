...
const { parsePathParameters, replaceParameterPath, getAllParameterVariable } = require('./external')
...
async function postmanToOpenApi (input, output, {
  ...
  for (let [i, element] of items.entries()) {
    ...
    const { globalVariable, pathVariable } = getAllParameterVariable(url, values, variable)
    ...
    if (valid) {
      ...
      // external func.
      paramsMeta = paramsMeta || { ...parsePathParameters(pathVariable, globalVariable) }
