...
// create fake fs for use within browser
const fakeFs = {promises:{ writeFile: () => Promise.resolve(undefined), readFile: () => Promise.resolve(undefined) }};
// if used within browser (window object is defined) import fakeFs instead of fs
// no change for use with Node.js
const { promises:{writeFile, readFile} } = typeof window === 'object' ? fakeFs : require('fs');
