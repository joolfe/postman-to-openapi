...
const setInObj = require("lodash.set")
...
async function postmanToOpenApi (input, output, {
  ...
  // inject custom json
  if(jsonInjectionFile) {
    let injectJson = JSON.parse(await resolveInput(jsonInjectionFile))
    openApi = injectCustomJson(openApi, injectJson)
  }
...
/**
 * injects custom json directly into the openapi doc
 * @param {*} originalJson - the openapi json
 * @param {*} injectJson - the json trying to inject
 * @returns 
 */
function injectCustomJson (originalJson, injectJson) {
  for (const nestedPath in injectJson) {
    const element = injectJson[nestedPath];
    setInObj(originalJson, nestedPath, element)
  }
