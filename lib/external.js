...
function getAllParameterVariable (url, global, collection) {
  const globalVariable = mergeCollectionToGlobalVariable(global, collection)
  const { variable: pathVariable } = replaceUrlPath(url, globalVariable)
  return { globalVariable, pathVariable }
}
